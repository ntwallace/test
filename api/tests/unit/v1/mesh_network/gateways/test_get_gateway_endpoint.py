...
def test_get_gateway_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/gateways/{uuid4()}')
...
def test_get_gateway_access_scope_responses(
    ...
    response = test_client.get(f'/v1/gateways/{gateway.gateway_id}')

    assert response.status_code == response_code


def test_get_gateway_success_response(
...
def test_get_gateway_when_gateway_doesnt_exist(
    admin_all_access_token_data: AccessTokenData,
    gateways_service_mock: Mock,
):
    app.dependency_overrides[get_access_token_data] = lambda: admin_all_access_token_data
...
def test_get_gateway_when_accessing_unauthorized_gateway(
    ...
    response = test_client.get(f'/v1/gateways/{unauthorized_gateway.gateway_id}')
