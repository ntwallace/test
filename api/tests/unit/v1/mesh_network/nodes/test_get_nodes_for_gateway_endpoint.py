...
def test_get_nodes_for_gateway_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/gateways/{uuid4()}/nodes')
...
def test_get_nodes_for_gateway_access_scope_responses(
    ...
    response = test_client.get(f'/v1/gateways/{gateway.gateway_id}/nodes')

    assert response.status_code == response_code


def test_get_nodes_for_gateway_success_response(
...
def test_get_nodes_for_gateway_when_no_nodes(
    admin_all_access_token_data: AccessTokenData,
    gateways_service_mock: Mock,
    nodes_service_mock: Mock,
    gateway: Gateway
    ...
    gateways_service_mock.get_gateway_by_gateway_id.return_value = gateway
    app.dependency_overrides[get_gateways_service] = lambda: gateways_service_mock
    nodes_service_mock.get_nodes_by_gateway_id.return_value = []
    app.dependency_overrides[get_nodes_service] = lambda: nodes_service_mock
...
def test_get_nodes_for_gateway_when_gateway_doesnt_exist(
    admin_all_access_token_data: AccessTokenData,
    gateways_service_mock: Mock
):
    app.dependency_overrides[get_access_token_data] = lambda: admin_all_access_token_data
...
def test_get_nodes_for_gateway_when_location_is_unauthorized(
    ...
    response = test_client.get(f'/v1/gateways/{unauthorized_gateway.gateway_id}/nodes')
