...
def test_get_electric_sensors_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/electric-sensors?gateway_id={uuid4()}')
...
def test_get_electricity_sensors_access_scope_responses(
    ...
    electric_sensors_service_mock.filter_by.return_value = electric_sensors
    ...
    response = test_client.get(f'/v1/electric-sensors?gateway_id={electric_sensors[0].gateway_id}')
    
    assert response.status_code == response_code


def test_get_electric_sensors_success_response(
    ...
    response = test_client.get(f'/v1/electric-sensors?gateway_id={electric_sensor.gateway_id}')
...
def test_get_electric_sensors_without_panel_success_response(
    admin_all_access_token_data: AccessTokenData,
    electric_sensors_service_mock: Mock,
    gateways_service_mock: Mock,
    gateway: Gateway,
...
def test_get_electric_sensors_when_electric_sensors_dont_exist(
    admin_all_access_token_data: AccessTokenData,
    electric_sensors_service_mock: Mock,
    electric_panels_service_mock: Mock,
    gateways_service_mock: Mock,
    ...
    electric_sensors_service_mock.filter_by.return_value = []
    app.dependency_overrides[get_electric_sensors_service] = lambda: electric_sensors_service_mock
...
def test_get_electric_sensors_when_gateway_doesnt_exist(
    token_data_with_access_scopes: Callable,
    electric_sensors_service_mock: Mock,
    gateways_service_mock: Mock,
    electric_sensor: ElectricSensor
):
    token_data = token_data_with_access_scopes([AccessScope.ELECTRICITY_MONITORING_READ])
    app.dependency_overrides[get_access_token_data] = lambda: token_data
...
def test_get_electric_sensors_when_location_is_unauthorized(
...
def test_get_electric_sensors_uses_filters(
    admin_all_access_token_data: AccessTokenData,
    electric_sensor: ElectricSensor,
    electric_sensors_service_mock: Mock,
    electric_panels_service_mock: Mock,
    ...
    response = test_client.get(f'/v1/electric-sensors?gateway_id={electric_sensor.gateway_id}&name={electric_sensor.name}&duid={electric_sensor.duid}')
    
    electric_sensors_service_mock.filter_by.assert_called_once_with(
