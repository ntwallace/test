...
def test_get_hvac_zone_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/hvac-zones/{uuid4()}')
...
def test_get_hvac_zone_access_scopes(
    ...
    response = test_client.get(f'/v1/hvac-zones/{hvac_zone.hvac_zone_id}')

    assert response.status_code == response_code


def test_get_hvac_zone_success_response(
...
def test_get_hvac_zone_when_hvac_zone_doesnt_exist(
    admin_all_access_token_data: AccessTokenData,
    hvac_zones_service_mock: Mock,
):
    app.dependency_overrides[get_access_token_data] = lambda: admin_all_access_token_data
