...
from app.v1.dependencies import get_api_key_access_scopes_helper, get_api_key_data, get_hvac_zones_service, get_access_token_data
...
def test_create_hvac_zone_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.post('/v1/hvac-zones')
...
def test_create_hvac_zone_access_scopes(
    ...
    response = test_client.post('/v1/hvac-zones', json={
        'name': 'Test HVAC Zone',
...
def test_create_hvac_zone_success_response(
...
def test_create_hvac_zone_sucess_when_api_key_auth(
    api_key: APIKey,
    ...
    api_key_access_scopes_helper_mock.get_all_access_scopes_for_api_key.return_value = [AccessScope.ADMIN, AccessScope.HVAC_WRITE]
...
def test_create_hvac_zone_when_exists(
    admin_all_access_token_data: AccessTokenData,
    hvac_zones_service_mock: Mock,
    hvac_zone: HvacZone
):
    ...
    hvac_zones_service_mock.create_hvac_zone.side_effect = ValueError
    app.dependency_overrides[get_hvac_zones_service] = lambda:  hvac_zones_service_mock
