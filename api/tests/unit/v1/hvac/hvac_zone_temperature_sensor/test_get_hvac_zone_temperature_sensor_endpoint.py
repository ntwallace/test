...
def test_get_hvac_zone_temperature_sensor_is_unauthorized_without_token():
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/hvac-zone-temperature-sensors/{uuid4()}')
...
def test_get_hvac_zone_temperature_sensor_access_scopes(
    ...
    response = test_client.get(f'/v1/hvac-zone-temperature-sensors/{hvac_zone_temperature_sensor.hvac_zone_temperature_sensor_id}')

    assert response.status_code == response_code


def test_get_hvac_zone_temperature_sensor_success_response(
...
def test_get_hvac_zone_temperature_sensor_when_temperature_sensors_dont_exist(
    admin_all_access_token_data: AccessTokenData,
    hvac_zone_temperature_sensors_service_mock: Mock,
):
    app.dependency_overrides[get_access_token_data] = lambda: admin_all_access_token_data
