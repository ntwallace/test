...
def test_list_user_effective_access_scopes_unauthorized(sample_user_id: UUID):
    app.dependency_overrides[get_access_token_data] = get_access_token_data
    response = test_client.get(f'/v1/auth/users/{sample_user_id}/effective-access-scopes')
...
def test_list_user_effective_access_scopes_access_scope_responses(
    access_scopes: List[AccessScope],
    response_code: int,
    token_data_with_access_scopes: Callable,
    mock_users_service: Mock,
    mock_user_access_scopes_helper: Mock,
    sample_user_id: UUID,
    sample_user: User
):
    ...
    app.dependency_overrides[get_user_access_scopes_helper] = lambda: mock_user_access_scopes_helper

    mock_users_service.get_user_by_user_id.return_value = sample_user
    expected_scopes = [AccessScope.ADMIN, AccessScope.USERS_READ]
    mock_user_access_scopes_helper.get_all_access_scopes_for_user.return_value = expected_scopes
...
def test_list_user_effective_access_scopes_success(
    mock_users_service: Mock,
    mock_user_access_scopes_helper: Mock,
    ...
    mock_user_access_scopes_helper.get_all_access_scopes_for_user.assert_called_once_with(sample_user_id)


def test_list_user_effective_access_scopes_user_not_found(
    mock_users_service: Mock,
