locals {
  environment             = "sandbox"
  app_name                = "sandbox-powerx-api"
  upstream_basis          = "sandbox-basis"
  upstream_data_pipelines = "sandbox-data-pipelines"
  app                     = terraform.workspace == "default" ? local.app_name : "${local.app_name}-${terraform.workspace}"

  dashboard_web_domain = "sandbox.powerx.co"
  powerx_api_domain    = "api-v2.${local.dashboard_web_domain}"
  logo_s3_bucket_name  = "sandbox-dashboard-media"
